...
/// ];
///
/// let polygon = Polygon::new(line_string.clone(), rvec![]);
///
/// // Point in Point
...
mod test {
    ...
    fn linestring_contains_point() {
        let line_string = LineString::from(rvec![(0., 0.), (3., 3.)]);
    ...
    fn linestring_fully_contained_in_polygon() {
        let poly = Polygon::new(
            LineString::from(rvec![(0., 0.), (5., 0.), (5., 6.), (0., 6.), (0., 0.)]),
            rvec![],
        );
        let ls = LineString::from(rvec![(3.0, 0.5), (3.0, 3.5)]);
        assert!(poly.contains(&ls));
    }
    /// Tests: Point in LineString
    #[test]
    fn empty_linestring_test() {
        let linestring = LineString::new(RVec::new());
        assert!(!linestring.contains(&Point::new(2., 1.)));
    }
    #[test]
    fn linestring_point_is_vertex_test() {
        let linestring = LineString::from(rvec![(0., 0.), (2., 0.), (2., 2.)]);
    ...
    fn polygon_with_one_point_test() {
        let linestring = LineString::from(rvec![(2., 1.)]);
        let poly = Polygon::new(linestring, RVec::new());
    ...
    fn point_in_polygon_with_ray_passing_through_a_vertex_test() {
        let linestring = LineString::from(rvec![(1., 0.), (0., 1.), (-1., 0.), (0., -1.)]);
        let poly = Polygon::new(linestring, RVec::new());
    ...
    fn point_polygon_with_inner_test() {
        let linestring = LineString::from(rvec![(0., 0.), (2., 0.), (2., 2.), (0., 2.), (0., 0.)]);
        let inner_linestring = LineString::from(rvec![
            [0.5, 0.5],
            [1.5, 0.5],
            [1.5, 1.5],
            [0.0, 1.5],
        ...
        let poly = Polygon::new(linestring, rvec![inner_linestring]);
    ...
    fn empty_multipolygon_test() {
        let multipoly = MultiPolygon::new(RVec::new());
        assert!(!multipoly.contains(&Point::new(2., 1.)));
    }
    #[test]
    fn empty_multipolygon_two_polygons_test() {
        let poly1 = Polygon::new(
            LineString::from(rvec![(0., 0.), (1., 0.), (1., 1.), (0., 1.), (0., 0.)]),
            RVec::new(),
        );
        let poly2 = Polygon::new(
            LineString::from(rvec![(2., 0.), (3., 0.), (3., 1.), (2., 1.), (2., 0.)]),
            RVec::new(),
        );
    ...
    fn empty_multipolygon_two_polygons_and_inner_test() {
        ...
        let poly2 = Polygon::new(
            LineString::from(rvec![(9., 0.), (14., 0.), (14., 4.), (9., 4.), (9., 0.)]),
            RVec::new(),
        );
    ...
    fn linestring_in_polygon_with_linestring_is_boundary_test() {
        let linestring = LineString::from(rvec![(0., 0.), (2., 0.), (2., 2.), (0., 2.), (0., 0.)]);
        let poly = Polygon::new(linestring.clone(), RVec::new());
        assert!(!poly.contains(&linestring));
        assert!(!poly.contains(&LineString::from(rvec![(0., 0.), (2., 0.)])));
        assert!(!poly.contains(&LineString::from(rvec![(2., 0.), (2., 2.)])));
        assert!(!poly.contains(&LineString::from(rvec![(0., 2.), (0., 0.)])));
    }
    #[test]
    fn linestring_outside_polygon_test() {
        let linestring = LineString::from(rvec![(0., 0.), (2., 0.), (2., 2.), (0., 2.), (0., 0.)]);
        let poly = Polygon::new(linestring, RVec::new());
        assert!(!poly.contains(&LineString::from(rvec![(1., 1.), (3., 0.)])));
        assert!(!poly.contains(&LineString::from(rvec![(3., 0.), (5., 2.)])));
    }
    #[test]
    fn linestring_in_inner_polygon_test() {
        ...
        assert!(!poly.contains(&LineString::from(rvec![(2., 2.), (3., 3.)])));
        assert!(!poly.contains(&LineString::from(rvec![(2., 2.), (2., 5.)])));
        assert!(!poly.contains(&LineString::from(rvec![(3., 0.5), (3., 5.)])));
    }
    #[test]
    ...
    fn linestring_in_line_test() {
        ...
        // linestring0 in line
        let linestring0 = LineString::from(rvec![(1, 11), (10, 20), (15, 25)]);
        // linestring1 starts and ends in line, but wanders in the middle
        let linestring1 = LineString::from(rvec![(1, 11), (20, 20), (15, 25)]);
        // linestring2 is co-linear, but extends beyond line
        let linestring2 = LineString::from(rvec![(1, 11), (10, 20), (40, 50)]);
        // no part of linestring3 is contained in line
        let linestring3 = LineString::from(rvec![(11, 11), (20, 20), (25, 25)]);
        // a linestring with singleton interior on the boundary of the line
        let linestring4 = LineString::from(rvec![(0, 10), (0, 10), (0, 10)]);
        // a linestring with singleton interior that is contained in the line
        let linestring5 = LineString::from(rvec![(1, 11), (1, 11), (1, 11)]);
    ...
    fn line_in_polygon_test() {
        ...
        let poly0 = Polygon::new(linestring0, RVec::new());
        ...
        let poly1 = Polygon::new(linestring1, RVec::new());
    ...
    fn line_in_polygon_edgecases_test() {
        ...
        let poly = Polygon::new(linestring0, RVec::new());
    ...
    fn line_in_linestring_test() {
        ...
        let linestring0 = LineString::from(rvec![(0., 0.5), (0.5, 0.5), (3., 3.)]);
        // line0 is contained in the last three segments
        let linestring1 = LineString::from(rvec![
            (0., 0.5),
            (0.5, 0.5),
            (1.2, 1.2),
            (1.5, 1.5),
        ...
        let linestring2 = LineString::from(rvec![
            (0., 0.5),
            (0.5, 0.5),
            (1.2, 1.2),
            (1.5, 0.),
